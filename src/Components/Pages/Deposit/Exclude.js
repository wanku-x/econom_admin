    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        this.setState({ loading: true }, () => {
          requestPOST('/api/v1/exclude_deposit_money/', values).then((result)=>{
            if (result.success) {
              message.success('Команда сняла инвестиций на '+ values.exclude_amount + '$');
    ...
    return (
      <Form onSubmit={this.onSubmit}>
        <FormItem
          ...
          label="Сумма"
        >
          {getFieldDecorator('exclude_amount', {
            initialValue: 0 ,
            normalize: this.changeDeposit,
            rules: [{ required: true, validator: this.checkDeposit }],
          })(
            <Input
              suffix="$"
              size="large"
...
export default Form.create()(Exclude);