    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        this.setState({ loading: true }, () => {
          requestPOST('/api/v1/invest_money/', values).then((result)=>{
            if (result.success) {
              message.success('Команда инвестировала '+ values.invest_amount + '$');
    ...
    return (
      <Form onSubmit={this.onSubmit}>
        <FormItem
          ...
          label="Сумма"
        >
          {getFieldDecorator('invest_amount', {
            initialValue: 0 ,
...
export default Form.create()(Invest);